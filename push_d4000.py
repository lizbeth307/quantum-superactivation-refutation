"""push_d4000.py — Push to d=4000 with optimized memory usage."""
...
if __name__ == '__main__':
    ...
    # d=4000: rho8 x I_500/500 -> 2x2000
    for name, base, k in [('d=4000', rho8, 500), ('d=5000', rho10, 500)]:
        rho = np.kron(base, np.eye(k)/k)
        dA = 2; dB = rho.shape[0] // 2; d = dA * dB
        print(f"{name} ({dA}x{dB}): 15 cores x 20 bases (memory-safe)...", flush=True)
        t0 = time.time()
        # Use fewer workers for memory
        args = [(rho, dA, dB, seed, 20) for seed in range(15)]
